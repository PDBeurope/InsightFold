...
| Primary reference | `CLAUDE.md` — Homodimer Confidence Scoring skill (formulas, API contracts, thresholds) |
...
Three agents run in parallel once the architecture blueprint is finalised. All three are governed by the `Homodimer Confidence Scoring` skill in `CLAUDE.md` — formulas must not be inferred from training data.
...
- PAE quadrant extraction must follow the exact pattern in `CLAUDE.md` (sort `chains` by `label_asym_id`, use `sequenceStart`/`sequenceEnd`).
...
- Use `CLAUDE.md` sigmoid constants verbatim; do not derive from spec prose.
...
**Primary skill for Section 6:** `skills/molviewspec-rendering/SKILL.md`.

Key constraints:
- Section 6 entirely inside `try/except ImportError`.
- Traffic-light thresholds from `CLAUDE.md` `THRESHOLDS` dict — do not use spec prose values.
...
| Check | Detail |
|---|---|
| Formula correctness | Cross-reference every `compute_*` function against `CLAUDE.md` primitives (`d0_func`, `ptm_func`, PAE cutoffs, sigmoid coefficients) |
| Variable handoff | Confirm each cell's inputs exist as outputs of a prior cell; no undefined references |
| Edge-case coverage | Verify all 9 rows of the edge-case table in `CLAUDE.md` are handled (zero contacts → floor values, GLY→CA, multi-model filter, etc.) |
...
Use `skills/molviewspec-rendering/SKILL.md` as the canonical MolViewSpec procedure. The notes below are historical homodimer-specific constraints and additional checks.
