...
# Integrated Implementation Updates

# Additional Specification Updates

## Data Retrieval Sources

### Remote Structure Retrieval

The notebook must support direct retrieval of model and restraint files from PDBe entry-file endpoints.
...
```text
https://www.ebi.ac.uk/pdbe/entry-files/download/{PDB_ID}.cif
```

Example:

```text
https://www.ebi.ac.uk/pdbe/entry-files/download/9l1v.cif
```

---

#### NMR Restraint File Endpoint

Template:

```text
https://www.ebi.ac.uk/pdbe/entry-files/download/{PDB_ID}_nmr-data.str
```

Example:

```text
https://www.ebi.ac.uk/pdbe/entry-files/download/9l1v_nmr-data.str
```
...
The notebook must also support:
- locally provided mmCIF model files,
- locally provided NMR-STAR restraint files.

The user may therefore choose between:
- remote retrieval using a PDB ID,
- or local filesystem paths.

---

# Structure Parsing Requirements

## Canonical Structure Parser

The implementation must use the `gemmi` Python module for coordinate parsing.
...
The Gemmi-based structure parser must support:
...
- atom lookup table construction,
...
must originate from Gemmi-parsed structure objects.

---

# Ambiguous Distance Restraints — Expanded Specification

## Conceptual Definition

Ambiguous distance restraints occur when:
- multiple atom-pair assignments are possible,
- and the deposited experimental evidence does not uniquely determine the correct atom pair.

This commonly occurs:
- in methyl groups,
- chemically equivalent atoms,
- or unresolved assignments.

---

## NMR-STAR Identification Rules

An ambiguous restraint is identified when:

1. Multiple rows share the same:
...
```text
_Gen_dist_constraint.Member_logic_code = OR
```
...
```text
5  5  . OR 1 56 LEU HD21 2 1 LIG H1 3.44 4.13 1 A 56  LEU HD21 HDy% B 500 LIG H1 H1 1
6  5  . OR 1 56 LEU HD22 2 1 LIG H1 3.44 4.13 1 A 56  LEU HD22 HDy% B 500 LIG H1 H1 1
7  5  . OR 1 56 LEU HD23 2 1 LIG H1 3.44 4.13 1 A 56  LEU HD23 HDy% B 500 LIG H1 H1 1
```
...
| Field | Meaning |
|---|---|
| `_Gen_dist_constraint.ID = 5` | logical restraint identifier |
| `Member_logic_code = OR` | ambiguous restraint semantics |
| multiple rows | alternative atom-pair realizations |

---

## Canonical Evaluation Behaviour

For ambiguous restraints:

1. Compute all candidate atom-pair distances.
2. Select the smallest measured distance.
3. Compare the selected distance against the allowed restraint bounds.
4. Use only this selected member for:
   - violation evaluation,
   - visualization,
   - reporting,
   - and local inspection.

All alternative members are discarded after evaluation in v1.
...
The notebook must provide widgets allowing users to choose between:

| Mode | Description |
|---|---|
| Remote Retrieval | fetch files using PDB ID |
| Local Files | use local mmCIF and NMR-STAR files |
...
Purpose:
- retrieve remote structure and restraint files.
...
### 2. Local Model File Widget

Purpose:
- provide path to local mmCIF model file.

Widget type:
- file-path input or file-upload widget.
...
### 3. Local Restraint File Widget

Purpose:
- provide path to local NMR-STAR restraint file.
...
### 4. Residue Navigation Widget

Purpose:
- transition from global density view into local evidence view.

Widget type:
- numeric residue selector.

Input:
- residue number.

Behaviour:
- selecting a residue updates:
  - local neighborhood,
  - local restraints,
  - local visualization state,
  - and camera focus using standard MolViewSpec behaviour.
...
The initial 3D visualization must display:

- the complete structure,
- rendered as cartoon representation,
- with residue colors mapped to restraint density.
...
## Initial Interpretation Goal

The initial visualization should immediately communicate:
- experimentally dense regions,
- sparse regions,
- and overall restraint-support distribution.

---

## Transition to Local View

The notebook transitions into local evidence mode when:

- a residue is selected in the sequence view,
- a residue number is entered in the residue widget,
- or a violation row is selected.

---

## Local View Behaviour

The local view must display:

- selected residue,
- neighboring residues within local radius,
- all restraints associated with selected residue,
- local violations,
- and reduced-opacity contextual residues.
...
```text
Global density view
→ user selects residue
→ local neighborhood computed
→ local restraint network rendered
→ local evidence inspection enabled
```